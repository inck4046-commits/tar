...
# app_gradio_multi_m9.py
from __future__ import annotations
from typing import Any, Dict, List, Tuple, Optional
from functools import lru_cache
import itertools, os, tempfile, time, math
...
from calc.buff import BUFFS_ALL
...
ENCH_VARIANTS = {"HP":{"hp":0.21,"atk":0.0,"def":0.0},
...
TWO_BUFF6   = ["HP40%","ATK40%","DEF40%","HP+ATK20%","HP+DEF20%","ATK+DEF20%"]

# ---- ui13 '최대 버프' 후보 규칙 ----
TB_PAIRS = [
    ("HP40%",     BUFFS_ALL.get("HP40%",  {"hp":0.40,"atk":0.0,"def":0.0})),
    ("ATK40%",    BUFFS_ALL.get("ATK40%", {"hp":0.0,"atk":0.40,"def":0.0})),
    ("DEF40%",    BUFFS_ALL.get("DEF40%", {"hp":0.0,"atk":0.0,"def":0.40})),
    ("HP+ATK20%", BUFFS_ALL.get("HP+ATK20%", {"hp":0.20,"atk":0.20,"def":0.0})),
    ("HP+DEF20%", BUFFS_ALL.get("HP+DEF20%", {"hp":0.20,"atk":0.0,"def":0.20})),
    ("ATK+DEF20%",BUFFS_ALL.get("ATK+DEF20%",{"hp":0.0,"atk":0.20,"def":0.20})),
]
def _filter_two_buff_candidates_by_buflbl(buf_lbl, tb_pairs):
    ...
    if not tb_pairs:
        return tb_pairs
    if label in {"HP+ATK20%","HP+DEF20%","ATK+DEF20%"}:
        return [(lab,pct) for (lab,pct) in tb_pairs if lab == label]
    if label in {"HP40%","ATK40%","DEF40%"}:
        return [(lab,pct) for (lab,pct) in tb_pairs if lab == label]
    if label == "ATK20%":
        wants = {"ATK40%","HP+ATK20%","ATK+DEF20%"}
        return [(lab,pct) for (lab,pct) in tb_pairs if lab in wants]
    if label == "DEF20%":
        wants = {"DEF40%","HP+DEF20%","ATK+DEF20%"}
        return [(lab,pct) for (lab,pct) in tb_pairs if lab in wants]
    if label == "HP20%":
        wants = {"HP40%","HP+ATK20%","HP+DEF20%"}
        return [(lab,pct) for (lab,pct) in tb_pairs if lab in wants]
    return tb_pairs

@lru_cache(maxsize=256)
def best_buff_label_filtered(typ: str, row_buf_label: str):
    cands = _filter_two_buff_candidates_by_buflbl(row_buf_label, TB_PAIRS)
    best_lab, best_M = None, -1.0
    for lab, _pct in cands:
        M, _meta = spec_max(typ, lab)
        if M > best_M:
            best_M = float(M); best_lab = lab
    return best_lab if best_lab else (row_buf_label if isinstance(row_buf_label, str) else "HP40%")

# ---- core calc ----
def _strip_jingak(t: str) -> str:
    return t.replace("(진각)","") if isinstance(t, str) else t

def _acc_rows(levels: List[int]|None = None) -> List[Dict[str, Any]]:
    df = accmod.df_acc
    if not levels:
        rows = df.to_dict("records")
    else:
        rows = df[df["lv"].isin(list(map(int, levels)))].to_dict("records")
    return rows
...
def _calc_core(typ: str, ench_pct: Dict[str,float], acc_row: Dict[str, Any],
               ...
               spirit_opts: List[Dict[str, Any]], manual_stats: Optional[Dict[str,int]] = None):
    if manual_stats is None:
        bs = get_base_stats(typ); aw = get_awakening_stat(typ)
    else:
        bs = dict(manual_stats); aw = {"hp":0,"atk":0,"def":0}
    C1 = {k: bs[k] + aw[k] for k in ("hp","atk","def")}
    C2 = {"hp": gemv*4*dist[0], "atk": gemv*dist[1], "def": gemv*dist[2]}
    C3 = {k: C1[k] + C2[k] for k in C1}
    pot = apply_potion({}); C4 = {k: C3[k] + pot.get(k,0) for k in C3}
...
def spec_max(typ: str, buf_label: str):
    gemv = 40
    stats = ["체력","공격력","방어력"]
    base = []
    for s1 in stats:
    ...
    for comb in base:
        for sub in ("체력40","공격력10","방어력10"):
            c = [dict(x) for x in comb]; c[-1]["stat"] = sub; expanded.append(c)

    acc_rows = accmod.df_acc.to_dict("records")
    ench_all = list(ENCH_VARIANTS.items())

    M = 0; best = {}
    for dist in GEM_DISTS:
        for acc in acc_rows:
            for ench_name, ench_pct in ench_all:
                for comb in expanded:
                    bib, ivel, final = _calc_core(_strip_jingak(typ), ENCH_VARIANTS[ench_name], acc, gemv, dist, buf_label, comb,
                                                  manual_stats=get_base_stats(_strip_jingak(typ)))
                    if bib > M:
                        M = int(bib)
                        best = {"젬분배":f"{dist[0]}/{dist[1]}/{dist[2]}","장신구":f"{int(acc.get('lv',0))} {acc.get('이름','')}","인첸트":ench_name}
    return M, best

@lru_cache(maxsize=1024)
def spec_max_fixed(typ: str, row_buf: str, mfix_type: str, mfix_buff: str, two_buff_all: bool):
    # type candidates
    if mfix_type == "모든 타입":
...
def _apply_dup_filters_df(df: pd.DataFrame, by_type: bool, by_buff: bool, by_acc: bool, by_spirit: bool, by_ench: bool, metric_key: str) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        if by_buff:   k.append( (r["버프"], r["최대 버프"]) )
...
def _estimate_runtime(types, buffs, gem_values, parsed_dists, enchs, rows, spirit, need_tar, mfix_type, mfix_buff, mfix_two_buff, max_cases:int):
    ...
    it = itertools.product(types, buffs, gem_values, parsed_dists, enchs)
    done = 0
    t0 = time.perf_counter()
    for typ, buf, gemv, dist, ench in it:
        for acc in rows[:1]:  # one acc per outer combo to diversify axes quickly
            _ = _calc_core(typ, ENCH_VARIANTS[ench], acc, int(gemv), dist, buf, spirit, manual_stats=get_base_stats(_strip_jingak(typ)))
            if need_tar:
                _ = spec_max_fixed(typ, buf, mfix_type, mfix_buff, bool(mfix_two_buff))
            done += 1
            if done >= sample_n: break
        if done >= sample_n: break
    t1 = time.perf_counter()
    per_row = max(1e-6, (t1 - t0) / max(1, done))
    # add 15% overhead for sorting/dedup/etc
...
def run_search(
    ...
    mfix_type: str, mfix_buff: str, mfix_two_buff: bool
):
    ...
    for typ, buf, gemv, dist, ench in itertools.product(types, buffs, gem_values, parsed_dists, enchs):
        for acc in rows:
            bib, ivel, final = _calc_core(typ, ENCH_VARIANTS[ench], acc, int(gemv), dist, buf, spirit,
                                          manual_stats=get_base_stats(_strip_jingak(typ)))
            row = {"타입": typ, "버프": buf, "젬수치": int(gemv), "젬분배": f"{dist[0]}/{dist[1]}/{dist[2]}",
                   ...
                   "1옵": spirit_strs["1옵"], "2옵": spirit_strs["2옵"], "3옵": spirit_strs["3옵"],
                   "4옵": spirit_strs["4옵"], "부가옵": sub_str}
            try:
                row["최대 버프"] = best_buff_label_filtered(_strip_jingak(typ), str(buf))
            except Exception:
                row["최대 버프"] = str(buf)
            results.append(row)
            count += 1
    ...
    df0 = pd.DataFrame(results)                      # 생성 행 (제한 반영)
    generated_n = len(df0)

    # ensure TAR% if needed
    if need_tar:
        Ms = []
        Mlab = []
        for _, r in df0.iterrows():
            M, used_lab = spec_max_fixed(r["타입"], r["버프"], mfix_type, mfix_buff, bool(mfix_two_buff))
    ...
    status = f"OK | 원시 조합 {raw_total:,}  / 생성 {generated_n:,}  / 중복 후 {after_dedup_n:,}  / 표시 {final_n:,}  / 예상 {_fmt_seconds((gen_target/ max(1, generated_n)) * 0) }"
    # 위 status의 마지막은 자리표시자, 실제 ETA는 별도 필드로 반환
...
def run_opt_wrapper(*args):
    st, df, rawC, genC, dedC, finC, eta = run_search(*args)
    df = _mobile_trim_df(df, args[ -3 ])  # mobile_simple is placed 3rd from end among inputs (before TAR fix params)
    return st, df, rawC, genC, dedC, finC, eta



def run_all_wrapper(*args):
    # args ordering matches inputs in run_btn2.click
    st, df, rawC, genC, dedC, finC, eta = run_search(*args[:-1])  # last is mobile_simple2
    mobile_simple2 = args[-1]
    df = _mobile_trim_df(df, mobile_simple2)
    return st, df, rawC, genC, dedC, finC, eta
...
@media (max-width: 768px){
  ...
  .gradio-container label, .gradio-container button { font-size: 14px; }
    ...
    """
    with gr.Blocks(title="TAR 정령 자동 계산기", theme=gr.themes.Soft(), css=custom_css) as demo:
        with gr.Row():
        ...
        with gr.Row():
            ...
            ms_ench  = gr.CheckboxGroup(choices=list(ENCH_VARIANTS.keys()), value=list(ENCH_VARIANTS.keys()), label="인첸트 (다중)")
        ...
        with gr.Tabs():
            ...
            with gr.Tab("모든 경우"):
                ...
                csv_file = gr.File(label="CSV 다운로드(전체)", interactive=False)
                ...
                def run_all_and_csv(types, buffs, gem_values, dists, enchs, acc_levels, acc_names,
                    ...
                    # 모바일 간단 보기 적용
                    df_show = _mobile_trim_df(df_show, mobile_simple2)
                    with tempfile.NamedTemporaryFile("w", delete=False, suffix=".csv", prefix="all_cases_", encoding="utf-8-sig", dir=BASE_DIR) as tmp:
                        df.to_csv(tmp.name, index=False, encoding="utf-8-sig")
                        saved = tmp.name
                    fin_show = int(len(df_show)) if hasattr(df_show, "__len__") else int(finC)
                    return st, df_show, saved, int(rawC), int(genC), int(dedC), fin_show, eta
                ...
                run_btn2.click(
                    ...
                    outputs=[status2, table2, csv_file, cnt_raw2, cnt_gen2, cnt_ded2, cnt_fin2, eta_box2],
                )
