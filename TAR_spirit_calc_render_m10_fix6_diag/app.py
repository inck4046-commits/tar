...
def build_ui():
    ...
    with gr.Blocks(title=TITLE, theme=gr.themes.Soft(), css=custom_css) as demo:
        ...
        with gr.Tabs():
            with gr.Tab("환경 점검"):
                gr.Markdown("**서버/데이터 점검 — 1조합만 계산합니다. 실패하면 상태에 스택트레이스가 찍힙니다.**")
                test_btn = gr.Button("테스트 실행(1조합)")
                test_status = gr.Textbox(label="테스트 상태", interactive=False)
                test_df = gr.Dataframe(label="테스트 결과 1행", interactive=False, wrap=True, row_count=(1,"dynamic"))
                def run_test_once():
                    try:
                        dfa = accmod.df_acc
                        if dfa.empty:
                            raise RuntimeError("calc/accessory.py의 df_acc가 비었습니다.")
                        row = dfa.iloc[0].to_dict()
                        acc_row = {"lv": int(row.get("lv", 0)), "이름": str(row.get("이름","")),
                                   "hp%": float(row.get("hp%", 0)), "atk%": float(row.get("atk%", 0)),
                                   "def%": float(row.get("def%", 0))}
                        spirit = [
                            {"stat":"체력","type":"%","slot":1},
                            {"stat":"공격력","type":"%","slot":2},
                            {"stat":"방어력","type":"%","slot":3},
                            {"stat":"체력","type":"+","slot":4},
                            {"stat":"체력40","type":"부가옵","slot":5},
                        ]
                        bib, ivel, final = _calc_core("체", ENCHANTS["HP"], acc_row, 37, (5,0,0), "HP40%", spirit)
                        row_out = {"타입":"체","버프":"HP40%","젬수치":37,"젬분배":"5/0/0","인첸트":"HP",
                                   "장신구": f"{acc_row['lv']} {acc_row['이름']}",
                                   "HP":final["hp"],"ATK":final["atk"],"DEF":final["def"],"이벨":ivel,"비벨":int(bib)}
                        import pandas as pd
                        df = pd.DataFrame([row_out])
                        return "OK(1행 계산 성공)", df
                    except Exception as e:
                        import traceback, pandas as pd
                        tb = traceback.format_exc()
                        print("SANITY_TEST_ERROR:", e, "\n", tb)
                        return "ERROR: " + str(e) + "\n" + tb[-2000:], pd.DataFrame()
                test_btn.click(lambda : run_test_once(), inputs=None, outputs=[test_status, test_df])
            ...
            with gr.Tab("최적화"):
                ...
                all_cases_state1 = gr.State(False)
                ...
                run_btn.click(
                    ...
                    inputs=[ms_types, ms_buffs, ms_gemv, ms_dists, ms_ench, ms_acc_levels, ms_acc_names,
                            ...
                            do_tar, topk, sort_key, all_cases_state1, max_cases,
